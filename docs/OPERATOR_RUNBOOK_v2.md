# OPERATOR RUNBOOK: PICTOLABS BOOTH #1 (VERSION 2.0)
## Field Operations, Emergency Recovery & Incident SOP Handbook

> **Target Unit:** Commercial Photobooth #1 (Mall / Cinema Enclosure)  
> **Audience:** Field operators, on-site mall staff, and duty technicians  
> **Classification:** Operational Standard Operating Procedure (SOP)  
> **Security Notice:** No credentials or passwords are stored in this document. Refer to Credential Sheet maintained by Operations.  
...
| :---: | :---: | :--- | :--- |
| **v1.0** | September 10, 2026 | Initial Release | Antigravity Operations |
| **v2.0** | September 10, 2026 | **Security Hardening** (all secrets replaced with "Refer to Credential Sheet maintained by Operations")<br>**Severity 0 Added** (revenue blocking + active customer impact)<br>**Compensation Workflow Added** (reprint, softfile, and refund flows) | Antigravity Operations |
...
- [ ] Test the live viewfinder on screen:
  ...
  - [ ] Enter the 6-digit technician PIN. *(Refer to Credential Sheet maintained by Operations.)*
...
- [ ] Empty the paper scrap bin:
  - [ ] Pull out the scrap box at the bottom front of the printer.
...
- [ ] Open the admin dashboard on your technician phone:
  - [ ] Login using authorized staff account. *(Refer to Credential Sheet maintained by Operations.)*
...
- [ ] Lock the rear door securely and keep Key #1 in the operator pouch.
...
- [ ] Enter the 6-digit PIN. *(Refer to Credential Sheet maintained by Operations.)*
...
## Incident logging
- [ ] Open the physical booth logbook kept inside the rear door pocket.
...
## Payment Success But Print Failed (CRITICAL REVENUE INCIDENT)

### Symptoms
- Customer bank account was debited (e.g. IDR 35,000 deducted), photos were taken on screen, but the printer did not eject the photo strips.
- Kiosk displayed an unexpected error screen or returned to the Welcome Screen without printing.
- Customer is waiting in front of the booth expecting their physical prints.

### Possible Causes
1. Printer ran out of paper or ribbon roll mid-print.
2. Mechanical paper jam occurred inside the internal cutter or roller assembly.
3. Windows Print Spooler (`spoolsv.exe`) froze or crashed during composite transfer.
4. USB communication cable disconnected during print rendering.

### Resolution Steps
1. **Acknowledge and Reassure the Customer (Immediate):**
   - Approach the customer immediately with polite, professional reassurance.
   - Example script: *"Halo Kak, mohon maaf atas kendalanya. Kami pastikan foto Kakak aman tersimpan di sistem kami. Kami bantu cetak ulang sekarang juga atau kami berikan kompensasi penuh."*
2. **Inspect Hardware Status:**
   - Tap the **Staff** button (top-right) -> enter the 6-digit PIN. *(Refer to Credential Sheet maintained by Operations.)*
   - Check the **Thermal Printer** badge:
     - If `PAPER_OUT`: Proceed to Printer Out of Paper SOP below.
     - If `JAM` or `ERROR`: Proceed to Printer Error SOP below.
3. **Execute the Reprint Flow:**
   - In the Operator Control Panel, locate the **Thermal Printer** section on the left.
   - Tap **Cetak Ulang Foto Terakhir**.
   - The printer will pull the already-composited 300 DPI layout from local storage and print a fresh set of strips.
   - If the local reprint button does not respond:
     - Open the Admin Dashboard on your smartphone (`https://admin.pictolabs.id`).
     - Go to **Support** -> find the customer's session (filter by time or order ID).
     - Tap **Kirim Ulang Cetak (Reprint)** -> select reason: `PRINT_DEFECT` or `PAPER_JAM`.
     - The print command will dispatch directly to the booth spooler.
4. **Execute the Softfile Delivery Flow:**
   - While the printer is reprinting, ensure the customer has their digital copies immediately:
     - In the Dashboard session view, tap **Salin Tautan Download**.
     - Send the direct softfile link to the customer via WhatsApp on the spot.
     - Verify the customer can open the link and preview their 4 photos and Live Photo video on their phone.
5. **Customer Compensation Flow:**
   - If the printer cannot be repaired within **5 minutes** (e.g. mechanical hardware failure or broken cutter):
     - **Option A (Full Immediate Cash/QRIS Refund):**
       - Collect customer's name, phone number, and GoPay/OVO/BCA account number.
       - Execute refund directly from the operational petty cash pouch or request immediate transfer from the Operations Lead.
       - Provide the digital softfiles for free via WhatsApp as courtesy.
     - **Option B (Complimentary Reprint Voucher):**
       - Issue a physical or digital 100% discount voucher code for any Pictolabs booth.
       - Send the high-resolution composite file directly to the customer's email or WhatsApp so they can print it at another partner booth.
   - Record the full incident details in the physical logbook: Customer Name, Phone, Transaction ID, Failure Reason, Action Taken, and Refund Amount.
...
5. In the kiosk screen, tap **Staff** -> enter PIN *(Refer to Credential Sheet maintained by Operations.)* -> tap **Perbarui Status**.
...
8. When the front LED turns solid green, tap **Staff** -> enter PIN *(Refer to Credential Sheet maintained by Operations.)* -> **Uji Potong (Cut Test)**.
...
11. On kiosk screen, tap **Staff** -> enter PIN *(Refer to Credential Sheet maintained by Operations.)* -> verify status is `✓ SIAP`.
...
4. Open the Operator Control Panel:
   - Tap **Staff** button (top-right) -> enter PIN. *(Refer to Credential Sheet maintained by Operations.)*
...
3. If cellular router has no signal (e.g. basement mall area):
   - Switch to emergency smartphone hotspot:
     - Turn on Personal Hotspot on technician phone. *(Refer to Credential Sheet maintained by Operations for SSID and Password.)*
     - Access Operator Control Panel -> **Keluar ke Windows**.
     - Click Wi-Fi icon at bottom right of taskbar, connect to the backup hotspot.
...
4. If the link still fails:
   ...
   - Open the Admin Dashboard on technician phone.
...
1. Tap the **Staff** button (top-right) -> enter the 6-digit PIN. *(Refer to Credential Sheet maintained by Operations.)*
...
3. If mall power does not restore within 5 minutes:
   - Safely shut down the booth: tap **Staff** -> enter PIN *(Refer to Credential Sheet maintained by Operations.)* -> **Keluar ke Windows** -> Windows Start -> **Shut down**.
...
# 5. Escalation Matrix & Contact Sheet

## Operational Escalation Tiers

| Level | Severity Definition | Examples | Target Response Time | Authorized Action / Primary Contact |
| :---: | :--- | :--- | :---: | :--- |
| **Severity 0** | **Revenue Blocking + Active Customer Impact** | • Customer already paid but cannot continue<br>• Printer failure during paid session<br>• Booth unavailable during operating hours<br>• Payment accepted but photos unavailable | **Immediate** | Floor Operator performs immediate compensation / reprint protocol; alerts Operations Lead and Founder via direct voice call. |
| **Severity 1** | **Booth Down (No Revenue, No Active Customer)** | Total power dead outside customer session; printer hardware physically broken; camera sensor offline; continuous crash loop | **<30 Minutes** | On-site Field Technician dispatches; alerts Operations Lead. |
| **Severity 2** | **Degraded Operation** | Camera operating on webcam backup; printer low on paper (<50 sheets); cellular connection slow; single isolated payment dispute resolved | **<2 Hours** | Duty Field Technician handles during scheduled route. |
| **Severity 3** | **Minor / Cosmetic** | Customer typo on email address; minor smudge on touchscreen glass; non-blocking translation display issue | **Same Day (Closing)** | Floor Operator logs in daily closing report. |

## Incident Escalation Contact Procedure
1. For **Severity 0** and **Severity 1**:
   - Place the physical acrylic notice **"Dalam Pemeliharaan Singkat"** over the booth touchscreen.
   - Immediately initiate a **direct voice telephone call** to the Operations Lead or Founder (do not rely solely on WhatsApp chat).
   - State clearly:
     - Booth Name & Mall Location
     - Customer status (refund issued / waiting)
     - Error description and codes displayed on screen
     - Steps already attempted from Section 3
2. Post photographic evidence of the screen and rear cabinet status LEDs to the internal Operations WhatsApp Group.

---

# 6. Official Contact Sheet

> **Notice:** Phone numbers and emails below use operational placeholders. Refer to the internal physical Credential & Contact Card stored in the operations safe for active numbers.

| Role / Partner | Organization / Provider | Contact Person | Phone Number | Email / Portal |
| :--- | :--- | :--- | :--- | :--- |
| **Operations Lead** | Pictolabs Internal Operations | `[Ops Lead Name]` | `+62 811-xxxx-xxxx` | `ops@pictolabs.id` |
| **Founder / Executive Escalation** | Pictolabs Management | `[Founder Name]` | `+62 812-xxxx-xxxx` | `founders@pictolabs.id` |
| **Duty Field Technician** | Pictolabs Hardware Fleet | `[Technician Name]` | `+62 813-xxxx-xxxx` | `tech@pictolabs.id` |
| **Thermal Printer Vendor Support** | PT DNP Indonesia / Authorized Distributor | Technical Support Desk | `+62 21-xxxx-xxxx` | `support@dnp-distributor.co.id` |
| **DSLR Camera Vendor Support** | PT Datascrip / Canon Service Center | Canon Pro Desk | `+62 21-xxxx-xxxx` | `service@datascrip.co.id` |
| **Internet Service Provider** | Telkomsel / Indosat IoT Fleet Support | Corporate Account Manager | `188 / 0811-xxxx` | `iot.support@telkomsel.co.id` |
| **Cloud Hosting & Infrastructure** | DigitalOcean / Cloudflare Support | Enterprise Support Desk | N/A (Online Ticket) | `https://cloud.digitalocean.com/support` |
...
4. **Network Backup:**
   - [ ] 1x Backup 4G USB modem with active data balance.
...
- [ ] Discreet **Staff** button is accessible at top-right; 6-digit PIN *(Refer to Credential Sheet maintained by Operations)* opens Operator Control Panel.
- [ ] Cellular router signal strong (>3 bars 4G/5G LTE).
- [ ] Admin Dashboard displays Booth #1 as **ONLINE** with all 5 pillars green.
- [ ] Midtrans QRIS production mode verified: live test payment processed successfully.
